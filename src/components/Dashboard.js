...
function Dashboard({dashboardData,added,setadded}) {
    ...
    <Box  sx={{width:`calc(100% - 245px)`,background:'white'}}>
     <AppBar  elevation={0} sx={{width:`calc(100% - 245px)`,background:'white'}} className='appbar' >
      ...
      <Toolbar  className='appbar' >
        ...
        Date - {dateApi(dashboardData ? dashboardData[11]?.value:"null")}
  ...
  <CardContent>
    ...
    {open && <FormTwo open={open} handleOpen={handleOpen} handleClose={handleClose} added={added} setadded={setadded} />}
    </>
  )
}
